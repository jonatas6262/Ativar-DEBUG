...
function triggerX()
{
	...
	<hr></div>
	<p><button id="btnROP" type="button" onclick="initROP(true);" autofocus>Initialize HAN Debug PKG Enabler</button> | Auto-Close Browser <input type="checkbox" id="auto_close" name="aclose" checked="checked" onclick="autoclose();"/><span id="dex_txt" style="visibility:hidden"> | DEX mode<input type="checkbox" id="dex" name="DEX" disabled="" onclick="dex();"/></span></p>
	<p><button id="btnTrigger" disabled="" type="button" onclick="triggerX();">Enable Debug PKG</button><span id="reset" style="visibility:hidden"> | <button id="btnReset" type="button" onclick="disable_trigger();">Reset</button></span></p>
